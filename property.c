...
static void ke_poperty_str_get(sml_t* sml) {
	...
	if (narg == 0) {
		ke_str_prop_get_0par(sml);
	}
	else if (narg == 1) {
		ke_str_prop_get_1par(sml);
	}
	else if (narg == 2) {
		ke_str_prop_get_2par(sml);
	} else {
...
static void ke_poperty_matrix_set(sml_t* sml) {
	int narg = sml_get_args(sml);
	if (narg == 2) {
...
static void ke_poperty_str_set(sml_t* sml) {
	int narg = sml_get_args(sml);
	if (narg == 1) {
		ke_str_prop_set_1par(sml);
	}
	else if (narg == 2) {
		ke_str_prop_set_2par(sml);
	}
	else {
...
void ke_poperty_get(sml_t* sml) {
	...
	case KEV_MAT:
		ke_poperty_matrix_get(sml);
		break;
	case KEV_VEC:
		ke_poperty_vector_get(sml);
		break;
	case KEV_VEC_INT:
		ke_poperty_vector_int_get(sml);
		break;
	case KEV_STR:
		ke_poperty_str_get(sml);
		break;
	default:
...
void ke_poperty_set(sml_t* sml) {
	...
	switch (sml_get_type(tokp)) {
	case KEV_MAT:
		ke_poperty_matrix_set(sml);
		break;
	case KEV_VEC:
		ke_poperty_vector_set(sml);
		break;
	case KEV_VEC_INT:
		ke_poperty_vector_int_set(sml);
		break;
	case KEV_STR:
		ke_poperty_str_set(sml);
		break;
	default:
