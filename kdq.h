...
#define __KDQ_IMPL(type, SCOPE) \
	...
	SCOPE type *kdq_peek_##type(kdq_##type##_t *q) \
	{ \
		return q->count? &q->a[((q->count) + q->front - 1) & q->mask] : 0; \
	} \
...
#define KDQ_DECLARE(type) \
	...
	type *kdq_peek_##type(kdq_##type##_t *q); \
...
#define kdq_peek(type, q) kdq_peek_##type(q)
